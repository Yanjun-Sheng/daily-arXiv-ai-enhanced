...
### [1] [Estimating stellar metallicities from Gaia DR3 XP data using LAMOST DR10](https://arxiv.org/abs/2607.18707)
*Divyansh Srivastava,Andrzej Niedzielski,Rodolfo Smiljanic*

Main category: astro-ph.GA

TL;DR: The authors train an XGBoost regressor using LAMOST DR10 [Fe/H] labels and Gaia DR3 inputs to estimate stellar metallicities for ~1.2M AFGK stars, achieving ~0.05-0.07 dex accuracy—far outperforming Gaia's GSP-Phot—and recovering the Milky Way's broken radial metallicity gradient.


<details>
  <summary>Details</summary>
Motivation: Gaia DR3's GSP-Phot metallicities suffer from systematic biases, motivating the need for more accurate, homogeneous [Fe/H] estimates using only Gaia data that can be applied at large scale for Galactic chemical studies.

Method: Cross-matched LAMOST DR10 with Gaia DR3; trained a gradient-boosted decision-tree (XGBoost) regressor on 1.20M AFGK stars using only Gaia-derived inputs/proxies, with LAMOST spectroscopic [Fe/H] as labels. Validated on held-out LAMOST, GALAH DR4, APOGEE DR17, and 46 open clusters.

Result: MAE=0.052 dex, R²=0.94 on held-out test set (vs. 0.242 dex for GSP-Phot). Transfers well to GALAH (0.066 dex) and APOGEE (0.068 dex). Open cluster median difference = 0.041 dex. Recovers broken thin-disk gradient (inner +0.119, outer -0.058 dex/kpc, break ~5.9 kpc) and open-cluster gradient (-0.066 dex/kpc).

Conclusion: The model delivers accurate [Fe/H] estimates (0.05–0.07 dex) for AFGK stars with [Fe/H] ≳ -2.5; below this, predictions are lower bounds. The public catalogue and model enable reliable large-scale chemical studies of the Milky Way.
